...
실제 게임 화면이다. Play 버튼을 누르면 플레이어가 보는 화면을 확인한다.
...
- Console에 에러가 있는가
- Player 오브젝트가 있는가
- Player에 `PlayerController`가 붙어 있는가
- Game 창이 활성화되어 있는가
- 키보드 입력이 Game 창으로 들어가고 있는가

### 상호작용이 안 됨

확인할 것:

- Player에 `PlayerInteractor`가 붙어 있는가
- 대상 오브젝트에 상호작용 스크립트가 붙어 있는가
- 대상 근처에 충분히 가까이 갔는가
- Collider가 있는가
- 상호작용 키가 E인지 확인했는가
...
```text
상황: Play 버튼을 누르고 E 키로 PartBin에 상호작용하려고 했음
예상 동작: 플레이어가 부품을 들어야 함
실제 동작: 아무 반응이 없음
